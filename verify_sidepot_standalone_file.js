...
try {
  consoleLog('--- Testing Standalone Side Pot Logic ---');
  ...
  consoleLog(`P1 Won: ${p1Win}`);
  consoleLog(`P2 Won: ${p2Win}`);
  consoleLog(`P3 Won: ${p3Win}`);
  ...
  fs.writeFileSync('verify_sidepot_standalone_out.txt', log.join('\n'));

} catch (e) {
  fs.writeFileSync('verify_sidepot_standalone_out.txt', `ERROR: ${e.message}`);
}
