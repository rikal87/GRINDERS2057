...
export const RICH_GUY_DIALOGUE = {
  [CHAT_TRIGGERS.GAME_START]: {
    ...
    ko: [
      "여기가 고액권 테이블인가? 생각보다 소박하네.",
      "난 재미 보러 온 거지, 돈 벌러 온 게 아냐.",
      "누가 좀 제대로 된 자본을 가지고 있나 볼까.",
      "카드가 마음에 안 들면 아예 이 카지노를 사서 바꿔버릴 거니까.",
      "잔돈 좀 잃어줄 준비들 됐어?"
    ]
  },
  ...
  [CHAT_TRIGGERS.ELIMINATED_SELF]: {
    ...
    ko: [
      ...
      "지지. 더 중요한 약속이 있어서 말이야.",
      "칩은 너 가져. 그냥 기념품으로 치지.",
      "조만간 개인 전용기를 타고 다시 올게.",
      "귀여운 게임이었어."
    ]
  },
  ...
  [CHAT_TRIGGERS.WAITING]: {
    ...
    ko: [
      ...
      "똑딱똑딱. 나처럼 제국을 운영하는 사람은 1분 1초가 아깝다고."
    ]
  },
  ...
  [CHAT_TRIGGERS.WIN_HERO_CALL]: {
    ...
    ko: [
      ...
      "네 공격이 좀 주제를 모르는 것 같아서 내가 잡아줬지. 수익 창출 완료.",
  ...
  [CHAT_TRIGGERS.BLUFF_CAUGHT]: {
    ...
    ko: [
      ...
      "네 콜 범위 계산에 아주 예외적인 오류가 발생했군. 흥미로워."
    ]
  },
  ...
  [CHAT_TRIGGERS.FOLD_STRONG]: {
    ...
    ko: [
      "공격의 투자 수익률(ROI)이 영 별로군. 폴드.",
      "이번 건 패스하지. 변동성이 너무 커. 리스크가 과하군.",
      "좋은 패지만, 더 나은 거래 조건을 찾고 있어. 폴드.",
      "이런 푼돈은 너나 가져. 적선이다.",
      "폴드하지. 더 중요한 비즈니스가 많아서 말이지."
    ]
  }
};
