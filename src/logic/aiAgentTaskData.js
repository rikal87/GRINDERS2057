...
export const AI_TASK_DATA = [
  ...
  },
  {
    ...
    duration: 60,
    cooldown: 0,
    effect: [{ type: TASK_EFFECT_TYPE.ADD_BANKROLL, amount: 1000, maxMultiple: 2 }],
  },
  {
    id: 'shadow_work_2',
    tier: 2,
    type: 'AGENT_WORK',
    name_ko: "향상된 서브넷 섀도우 워크",
    ...
    duration: 60,
    cooldown: 0,
    effect: [{ type: TASK_EFFECT_TYPE.ADD_BANKROLL, amount: 2500, maxMultiple: 2 }],
    unlock: { type: PLAY_RECORD_STATS_TYPE.COST_LT_TOTAL, amount: 250 }
  },
  {
    id: 'shadow_work_3',
    tier: 3,
    type: 'AGENT_WORK',
    name_ko: "대역폭 탈취",
    name_en: "Bandwidth Siphon",
    get name() { return store.settings.language === 'en' ? this.name_en : this.name_ko; },
    desc_ko: '활성 상태 동안 매시간 10% 확률로 5,000~25,000CR을 탈취합니다. 보안망에 포착될 경우 작업이 즉시 차단됩니다.',
    desc_en: 'Siphons 5,000~25,000 CR every hour with a 10% success rate. Getting flagged by the subnet security will terminate the link immediately.',
    get desc() { return store.settings.language === 'en' ? this.desc_en : this.desc_ko; },
    desc_detail_ko: '남의 서브넷 대역폭을 조금 빌리는 것뿐입니다. 주인은 아마 눈치도 못 채겠지만... 걸리면 골치 아파지겠죠?',
    desc_detail_en: 'Just borrowing a bit of someone\'s subnet bandwidth. They probably won\'t even notice... unless the security nodes trip.',
    get desc_detail() { return store.settings.language === 'en' ? this.desc_detail_en : this.desc_detail_ko; },
    cost: 15,
    probability: 0.1,
    duration: 0,
    cooldown: 3 * 24 * 60,
    effect: [{ type: TASK_EFFECT_TYPE.ADD_BANKROLL, amount: 5000, maxMultiple: 5 }, { type: TASK_EFFECT_TYPE.PENALTY_AMOUNT, id: 'SECURITY_DETECTION', probability: 0.01, amount: 0 }],
  ...
  },
  {
    ...
    type: 'AGENT_WORK',
    name_ko: "서브넷 하이재킹",
    name_en: "Sub-net Hijacking",
    get name() { return store.settings.language === 'en' ? this.name_en : this.name_ko; },
    desc_ko: '활성 상태 동안 매시간 10% 확률로 25,000~50,000CR을 탈취합니다. 보안망에 포착될 경우 작업이 즉시 차단됩니다.',
    desc_en: 'Siphons 25,000~50,000 CR every hour with a 10% success rate. Getting flagged by the subnet security will terminate the link immediately.',
    get desc() { return store.settings.language === 'en' ? this.desc_en : this.desc_ko; },
    desc_detail_ko: '이건 단순한 "작업"이 아닙니다. 기업용 통신망을 통째로 낚아채는 일이죠. 꼬리가 길면 반드시 잡힙니다.',
    desc_detail_en: 'This isn\'t just "work" anymore. You\'re snatching corporate data streams. Keep it running too long, and they\'ll hunt you down.',
    get desc_detail() { return store.settings.language === 'en' ? this.desc_detail_en : this.desc_detail_ko; },
    cost: 20,
    probability: 0.1,
    duration: 0,
    cooldown: 3 * 24 * 60,
    effect: [{ type: TASK_EFFECT_TYPE.ADD_BANKROLL, amount: 25000, maxMultiple: 2 }, { type: TASK_EFFECT_TYPE.PENALTY_AMOUNT, id: 'SECURITY_DETECTION', probability: 0.03, amount: 0 }],
  ...
  // For end game (Maybe useful..?)
  {
    ...
    get name() { return store.settings.language === 'en' ? this.name_en : this.name_ko; },
    type: 'NETWORKING',
    desc_ko: '[The Orbit] 입장 권한을 탈취합니다.',
    desc_en: 'Access to [The Orbit]',
    get desc() { return store.settings.language === 'en' ? this.desc_en : this.desc_ko; },
    desc_detail_ko: '"더 오르빗"에 대한 입장 권한을 탈취합니다. 발각 시 시스템이 즉시 소각될 수 있습니다.',
    desc_detail_en: 'Attempting to access the "The Orbit." Detection will lead to a catastrophic system burnout.',
    get desc_detail() { return store.settings.language === 'en' ? this.desc_detail_en : this.desc_detail_ko; },
    cost: 50,
    probability: 0.02,
    duration: 24 * 60,
    ...
    effect: [{ type: TASK_EFFECT_TYPE.PENALTY_AMOUNT, id: 'SECURITY_DETECTION', probability: 0.01, amount: 0 }],
  },
];