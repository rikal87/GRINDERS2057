  ...
  <div class="safe-house v5">
    ...
    <transition name="fade">
      <div v-if="selectedSlotIndex !== null" class="v5-modal-overlay popup-overlay" style="z-index: 1000;" @click.self="selectedSlotIndex = null">
        <div class="v5-modal popup-modal" style="width: 90%; max-width: 800px; max-height: 80vh; display: flex; flex-direction: column;">
          <h2 class="glitch-text modal-title" style="margin-bottom: 20px;">
            EQUIP MODULE [SLOT {{ selectedSlotIndex + 1 }}]
          </h2>
          
          <div v-if="currentSlotItem" class="current-equipped" style="margin-bottom: 20px; text-align: center;">
            <button class="btn-cancel" style="width: 100%; border: 1px solid var(--neon-red); color: var(--neon-red);" @click="unequipCurrentSlot">
              UNEQUIP CURRENT MODULE
            </button>
          </div>

          <div style="flex: 1; overflow-y: auto;">
            <div v-if="availableInventoryItems.length > 0" class="items-grid">
              <div v-for="item in availableInventoryItems" :key="item.instanceId" class="item-card"
                :class="{ 'dimmed': !canEquipToSelectedSlot(item) }"
                @click="canEquipToSelectedSlot(item) && equipItemToSlot(item.instanceId)">
                <div class="card-header">
                  <span class="tier-tag" :class="'tier-' + item.tier">{{ item.tier }}</span>
                  <!-- <button class="sell-btn" @click.stop="sellItem(item)">SELL</button> -->
                </div>
                ...
                <div class="item-details">
                  ...
                  <div class="effects-mini">
                    <div v-for="eff in item.effect" :key="eff?.type" class="eff-badge">
                      <span class="eff-name">⚡ {{ eff?.type }}</span>
                    </div>
                  </div>
                  <div v-if="!canEquipToSelectedSlot(item)" class="equipped-overlay isEquipped" style="background: rgba(255,0,0,0.5);">
                    <span style="color: white; font-size: 0.8rem;">TIER EXCEEDS SLOT CAPACITY</span>
                  </div>
                  <div v-else
                    :class="{ 'equipped-overlay': true, 'isEquipped': store.onWorkTasks.some(t => t.instanceId === item.instanceId) }">
                    <span>{{ store.onWorkTasks.some(t => t.instanceId === item.instanceId) ? 'EQUIPPED IN ANOTHER SLOT' : 'EQUIP' }}</span>
                  </div>
                </div>
              </div>
            </div>
            <div v-else class="empty-stock" style="text-align: center; padding: 40px;">
              <h2>NO MODULES AVAILABLE</h2>
              <p>BUY AT THE BLACK MARKET</p>
            </div>
          </div>
          
          <div class="modal-actions" style="margin-top: 20px;">
            <button class="btn-cancel" @click="selectedSlotIndex = null">CLOSE</button>
          </div>
        </div>
      </div>
    </transition>
    ...
    <div class="v5-body" ref="swipeContainer" @scroll.passive="onSwipeScroll">
      ...
      <section class="v5-side-col" :class="{ 'mobile-show': mobileActiveTab === 'status' }">
        <div class="v5-side-scroll-container">
          <!-- CYBER_LINK TERMINAL DECK BANNER -->
          <div class="v5-panel cyberlink-banner-panel" @click="$emit('enter-terminal')">
            <div class="cyberlink-banner-inner">
              <div class="banner-left">
                <span class="pulse-cyan"></span>
                <span class="cyberlink-title">CYBER_LINK // NET_TERMINAL</span>
                <span class="cyberlink-sub">ACCESS HIGH-STAKES TABLES & AI NEXUS</span>
              </div>
              <button class="btn-connect-terminal">
                <span>CONNECT</span> ➔
              </button>
            </div>
          </div>
      ...
      <section class="v5-main-col" :class="{ 'mobile-show': mobileActiveTab === 'manage' }">
      ...
      <section class="v5-msg-col" :class="{ 'mobile-show': mobileActiveTab === 'comms' }">
...
const currentSlotItem = computed(() => {
  if (selectedSlotIndex.value === null) return null;
  return store.onWorkTasks.find(t => t.slotIndex === selectedSlotIndex.value) || store.onWorkTasks[selectedSlotIndex.value];
});

const availableInventoryItems = computed(() => {
  return store.ownedItems;
});

const canEquipToSelectedSlot = (item) => {
  if (selectedSlotIndex.value === null) return false;
  
  const slotTier = currentAgentSlots.value[selectedSlotIndex.value];
  if (!slotTier) return false;
  
  const slotLevel = parseInt(slotTier.replace('T', ''));
  const itemLevel = parseInt(item.tier.replace('T', ''));
  
  // E.g. Slot T2 (level 2) can hold T2 and T1. Item T1 (level 1) <= 2 is true.
  return itemLevel <= slotLevel;
};

const equipItemToSlot = (instanceId) => {
  if (selectedSlotIndex.value === null) return;
  const item = store.ownedItems.find(p => p.instanceId === instanceId);
  if (!item) return;

  // Remove if it's already in another slot
  const existingIdx = store.onWorkTasks.findIndex(t => t.instanceId === instanceId);
  if (existingIdx !== -1) {
    store.onWorkTasks.splice(existingIdx, 1);
  }

  // Remove whatever is currently in the selected slot
  const currentOccupantIdx = store.onWorkTasks.findIndex(t => t.slotIndex === selectedSlotIndex.value);
  if (currentOccupantIdx !== -1) {
    store.onWorkTasks.splice(currentOccupantIdx, 1);
  } else if (store.onWorkTasks[selectedSlotIndex.value]) {
    // legacy index fallback
    store.onWorkTasks[selectedSlotIndex.value] = null;
    store.onWorkTasks = store.onWorkTasks.filter(t => t !== null);
  }
  ...
  store.onWorkTasks.push({
    ...
    state: { status: 'ACTIVE' },
    progress: 0,
    slotIndex: selectedSlotIndex.value // explicitly link to slot
  });
  
  audioManager.playSFX('ui-click');
  selectedSlotIndex.value = null; // Close popup
};

const unequipCurrentSlot = () => {
  if (selectedSlotIndex.value === null) return;
...
const taskSlots = computed(() => {
  ...
  for (let i = 0; i < 4; i++) {
    const taskState = store.onWorkTasks.find(t => t.slotIndex === i) || store.onWorkTasks[i];
    const taskDef = taskState ? (taskState.task || AI_TASK_DATA.find(t => t.id === taskState.taskId)) : null;
...
import { getAgentSlots } from '../logic/store.js';

const equipItem = (instanceId) => {
  const item = store.ownedItems.find(p => p.instanceId === instanceId);
  if (item) {
    const isEquipped = store.onWorkTasks.findIndex(t => t.instanceId === instanceId);
    if (isEquipped > -1) {
      // Unequip
      store.onWorkTasks.splice(isEquipped, 1);
      audioManager.playSFX('ui-click');
      return;
    }
    
    // Equip
    const maxSlots = getAgentSlots().length;
    if (store.onWorkTasks.length >= maxSlots) {
       // Slots full, swap out the oldest one
       store.onWorkTasks.shift();
    }
...
const sellItem = (item) => {
  ...
  const equippedIdx = store.onWorkTasks.findIndex(t => t.instanceId === item.instanceId);
  if (equippedIdx !== -1) store.onWorkTasks.splice(equippedIdx, 1);
};
...
<style scoped>
.cyberlink-banner-panel {
  background: rgba(10, 15, 22, 0.95);
  border: 1px solid var(--neon-yellow) !important;
  box-shadow: 0 0 15px rgba(248, 239, 0, 0.25);
  cursor: pointer;
  margin-bottom: 1rem;
  transition: all 0.25s cubic-bezier(0.165, 0.84, 0.44, 1);
}

.cyberlink-banner-panel:hover {
  background: rgba(248, 239, 0, 0.1);
  box-shadow: 0 0 25px rgba(248, 239, 0, 0.5);
  transform: translateY(-2px);
}

.cyberlink-banner-inner {
  display: flex;
...
.btn-connect-terminal {
  ...
  align-items: center;
  gap: 0.5rem;
  box-shadow: 0 0 10px rgba(248, 239, 0, 0.4);
}
